...
class ExtraTuner(Converter, object):
    ...
    def getText(self):
        ...
        if service:
            ...
            tunerinfo = self.type == self.TUNERINFO and self.getTunerInfo(service)
            text = tunerinfo
        ...
        if service:
            ...
            if feinfo is not None:
                if feinfo:
                    ...
                    frequency = frontendData is not None and (frontendData.get('tuner_type') == 'DVB-S' or frontendData.get('tuner_type') == 'DVB-C') and str(frontendData.get('frequency') / 1000) + ' MHz'
